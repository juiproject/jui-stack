...
import com.effacy.jui.codeserver.gwt.JobEvent.CompileStrategy;
...
public class Recompiler {
    ...
    private final CompilerModuleDir outboxDir;
    ...
    public Recompiler(CompilerModuleDir outboxDir, String inputModuleName, Options options, UnitCache unitCache, MinimalRebuildCacheManager minimalRebuildCacheManager) {
     ...
     */
    public synchronized Job.Result initWithoutPrecompile(TreeLogger logger) throws UnableToCompleteException {
        ...
        try {
            ...
            // Creates the dummy pre-compilation directory which contains assets that can be
            // served to trigger a compilation.
            setUpPreCompileDir(compileDir, module, compileLogger);
     ...
     */
    private void setUpPreCompileDir(CompileDir compileDir, ModuleDef module, TreeLogger compileLogger) throws UnableToCompleteException {
        try {
            ...
            // We do this in case static assets are reqiured pre-bootstrap.
            writePublicResources(outputDir, module, compileLogger);

            // The initial nocache.js bootstrap that will be used prior to a fist
            // compilation. This will inject recompile.nocache.js
    ...
    public void forceNextRecompile() {
        // We only need to clear the summary. This forces a recompile. Usually the
        // caches will have been cleared so a full compilation is performed.
    ...
    private Job.Result compile(Job job) throws UnableToCompleteException {
        ...
        // Setup our broader context on the first compilation.
        if (compilesDone == 0) {
            System.setProperty ("java.awt.headless", "true");
            if (System.getProperty ("gwt.speedtracerlog") == null)
                System.setProperty ("gwt.speedtracerlog", outboxDir.getSpeedTracerLogFile ().getAbsolutePath ());
            compilerContext = compilerContextBuilder.unitCache (unitCache).build ();
        }

        // Create the next compilation directory and start a compilation.
        long startTime = System.currentTimeMillis();
        ...
        try {
            ...
            boolean success = _compile (compileLogger, compileDir, job);
            if (!success) {
    ...
    /**
     * Called by {@link #compile(Job)} to perform the actualy compilation.
     */
    private boolean _compile(TreeLogger compileLogger, CompileDir compileDir, Job job) throws UnableToCompleteException {
        ...
        // Notify the compilation type we are using.
    ...
    /**
     * Generates the recompile.nocache.js file to use when precompile is not on.
     */
    private static String generateModuleRecompileJs(ModuleDef module, TreeLogger compileLogger) throws UnableToCompleteException {
    ...
    public synchronized String getRecompileJs(TreeLogger logger) throws UnableToCompleteException {
        ModuleDef loadModule = loadModule(logger);
        return generateModuleRecompileJs(loadModule, logger);
    }

    private static void writeRecompileNoCacheJs(File outputDir, String moduleName, String content, TreeLogger compileLogger) throws UnableToCompleteException {
        try {
            Files.write (content, new File (outputDir.getCanonicalPath () + "/" + moduleName + ".recompile.nocache.js"), Charsets.UTF_8);
        } catch (IOException e) {
    ...
    private TreeLogger makeCompileLogger(CompileDir compileDir, TreeLogger parent) throws UnableToCompleteException {
        try {
    ...
    private ModuleDef loadModule(TreeLogger logger) throws UnableToCompleteException {
        ...
        // Resources are first sought from the supplied sources location(s) then from
        ...
        ModuleDef moduleDef = ModuleDefLoader.loadFromResources (logger, inputModuleName, resources, true);

        // var branch1 = logger.branch(TreeLogger.INFO, "Sources: ");
        // for (String s : moduleDef.getAllSourceFiles ()) {
        //     if (s.startsWith("com/effacy/jui/core/client/component/layout"))
        //         branch1.log (TreeLogger.INFO, s);
        // }
        
        // Log out the modules.
        // List<String> modules = new ArrayList<> (moduleDef.getInheritedModules ());
        // Collections.sort (modules);
        // StringBuilder moduleLog = new StringBuilder ();
        // modules.forEach (m -> {
        //     if (m.equals (inputModuleName))
        //         return;
        //     if (!moduleLog.isEmpty())
        //         moduleLog.append(',');
        //     moduleLog.append (m);
        // });
        // logger.branch( TreeLogger.INFO, "Modules used by " + inputModuleName + ":")
        //     .log (TreeLogger.INFO, moduleLog.toString());
        ...
        if (options.isIncrementalCompileEnabled()) {
            ...
            maybeOverrideConfig (moduleDef, "CssDeclaration.style", "stable");
        }
     ...
     */
    private Map<String, String> restrictPermutations(TreeLogger logger, ModuleDef moduleDef, Map<String, String> bindingProperties) {
     ...
     */
    private static String maybeSetBinding(TreeLogger logger, ModuleDef module, String propName, String newValue) {
    ...
    private void overrideConfig(ModuleDef module, String propName, String newValue) {
        if (!maybeOverrideConfig(module, propName, newValue))
            throw new RuntimeException("not found: " + propName);
    }

    private boolean maybeOverrideConfig(ModuleDef module, String propName, String newValue) {
    ...
    private static class InputSummary {
        ...
        @Override
        public int hashCode() {
            return Objects.hashCode (bindingProperties, moduleLastModified, resourcesLastModified, filenameHash);
        }
    }
    ...
    /**
     * Copies all the contents of the `public` directories in the module roots.
     * 
     * @param moduleOutputDir
     ...
     */
    void writePublicResources(File moduleOutputDir, ModuleDef module, TreeLogger compileLogger) throws UnableToCompleteException, IOException {
