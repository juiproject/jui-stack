 ...
 * @see FLine
 */
public class DomBuilderBuilder implements IEventBuilder<IDomInsertableContainer<?>> {
     ...
     */
    public DomBuilderBuilder(IDomInsertableContainer<?> root) {
        this.root = root;
    }

    /**
     * Just return the root container, since content is appended directly to it or
     * its descendants as events are handled.
     * 
     * @return the root container.
     */
    public IDomInsertableContainer<?> result() {
        return root;
    }
     ...
     */
    public DomBuilderBuilder topHeadingLevel(int level) {
     ...
     * @return this handler for chaining.
     */
    public DomBuilderBuilder semanticTags(boolean semanticTags) {
     ...
     * @return this handler for chaining.
     */
    public DomBuilderBuilder semanticLists(boolean semanticLists) {
    ...
    public void formatted(String text, FormatType... formats) {
        if (semanticTags) {
            // Build nested semantic tags for each format (e.g. <strong><em>text</em></strong>).
            IDomInsertableContainer<?> target = currentTarget();
            boolean handled = false;
            for (FormatType format : formats) {
                String tag = FormattedTextStyles.SEMANTIC_TAGS.get(format);
                if (tag != null) {
                    ElementBuilder el = Custom.$(target, tag);
                    target = el;
                    handled = true;
                }
            }
     ...
     * the indent level and list type. Creates, removes, or replaces nesting as
     * needed to support mixed ordered/unordered lists at different depths.
     */
