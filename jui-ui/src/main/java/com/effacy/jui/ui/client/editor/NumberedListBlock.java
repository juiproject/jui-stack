...
public class NumberedListBlock extends ListBlock {
     ...
     */
    public NumberedListBlock(IEditor editor, ContentBlock content) {
        super (BlockType.NUMBERED_LIST, editor, content);
    }
    ...
    protected boolean onBackSpaceKey(UIEvent event) {
        if (EditorSupport.insertionPoint () && (EditorSupport.positionOfCursor (contentEl ()) == 0)) {
            editor.onReplace (this, new ParagraphBlock(editor(), content().transform (BlockType.PARAGRAPH)), true);
            return true;
        }
        return false;
    }
    ...
    @Override
    protected void refresh(com.effacy.jui.ui.client.editor.model.ContentBlock content) {
