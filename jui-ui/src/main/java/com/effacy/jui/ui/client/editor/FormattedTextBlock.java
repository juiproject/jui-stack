 ...
 */
public abstract class FormattedTextBlock extends Block implements IFormattedText {
     ...
     */
    public FormattedTextBlock(BlockType type, IEditor editor, ContentBlock content) {
    ...
    protected Element createContainer() {
        ...
        this.container = new FormattedText (containerEl);
                ...
                v.traverse ((text,styles) -> {
                    if (styles.length == 0) {
                        ...
                        for (FormatType format : styles) {
                            String style = FormattedText.resolve (format);
                            if (style != null)
        ...
        if (DomGlobal.document.getSelection().rangeCount > 0) {
            ...
            if (e.clipboardData.files.length > 0) {
                ...
                if (n != null) {
                    ...
                    new FormattedText (n).populate (blk);
                    n.remove ();
    ...
    protected boolean onEnterKey(UIEvent event) {
        if (EditorSupport.isCursorAtEnd (contentEl ())) {
            ...
            // that (PA.02).
            editor ().onInsertNext (createSplitBlock (new ContentBlock (type).indent (this.indent)), true);
            ...
            editor ().onInsertPrior (createSplitBlock (new ContentBlock (type).indent (this.indent)), false);
        } else {
     ...
     * @return the block.
     */
    protected abstract Block createSplitBlock(ContentBlock content);
    ...
    public boolean canMerge(Block source) {
        return source.type ().is (BlockType.PARAGRAPH, BlockType.HEADING_1, BlockType.HEADING_2, BlockType.HEADING_3, BlockType.NUMBERED_LIST);
    }
