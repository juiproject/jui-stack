...
public class Textarea {
    ...
    public static ElementBuilder $(Consumer<ElementBuilder>... builder) {
        return DomBuilder.textarea (builder);
    }
     ...
     * 
     * @param rows
     *                the number of rows (not applied if less than or equal to 0).
     * @param cols
     *                the number of columns (not applied if less than or equal to
     *                0).
     * @param builder
     *                the builders to apply.
     * @return the newly created element.
     */
    @SafeVarargs
    public static ElementBuilder $(int rows, int cols, Consumer<ElementBuilder>... builder) {
        return DomBuilder.textarea (rows, cols,builder);
    }
     ...
     * @return the newly created element.
     */
    public static ElementBuilder $(IDomInsertableContainer<?> parent, int rows, int cols) {
        ElementBuilder el = $ (rows, cols);
        parent.insert (el);
        return el;
    }
}
