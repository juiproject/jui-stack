...
public class GalleryItemCreator {
    
    public static <R> Supplier<IGalleryItem<R>> supplier(BiConsumer<R,ExistingElementBuilder> builder) {
        return new Supplier<IGalleryItem<R>> () {

            @Override
            public IGalleryItem<R> get() {
                return new GalleryItem<R> () {
                    protected INodeProvider buildNode(Element el) {
                        return DomBuilder.el (el, root -> {
                            builder.accept (getRecord (), root);
                        }).build ();
                    }
                };
            }
