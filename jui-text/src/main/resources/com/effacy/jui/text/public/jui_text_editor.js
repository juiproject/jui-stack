/**
 * JS support for editor (transaction-based rich text editor).
 *
 ...
 */
EditorSupport._firstLeaf = function (n) {
    if (n == null)
        return null;
 ...
 */
EditorSupport._lastLeaf = function (n) {
    if (n == null)
        return null;
    while (n.childNodes && n.childNodes.length > 0)
 ...
 */
EditorSupport._nextLeaf = function (scope, n) {
    if (n == null)
        return null;
    if (n.nextSibling)
        return EditorSupport._firstLeaf(n.nextSibling);
 ...
 */
EditorSupport._cefAncestor = function (scope, node) {
 ...
 */
EditorSupport._childIndex = function (parent, child) {
 ...
 */
EditorSupport._lines = function (el, stop) {
    var lines = [""];
    EditorSupport._linesWalk(el, stop, lines);
    return lines;
}

EditorSupport._linesWalk = function (el, stop, lines) {
    if (el === stop)
        return true;
    if (el.nodeType === 3) {
    ...
    for (var i = 0; i < el.childNodes.length; i++) {
        if (EditorSupport._linesWalk(el.childNodes[i], stop, lines))
            return true;
    }
    return false;
}
 ...
 */
EditorSupport.charCount = function (el) {
    var lines = EditorSupport._lines(el, null);
    var count = 0;
 ...
 */
EditorSupport._offsetInBlock = function (blockEl, node, offset) {
    if (node.nodeType === 1 && offset < node.childNodes.length) {
        ...
        if (node === blockEl)
            return EditorSupport.charCount(blockEl);
        // Nested element: count text before it plus text inside it.
        var beforeLines = EditorSupport._lines(blockEl, node);
        var count = 0;
        for (var i = 0; i < beforeLines.length; i++) {
            if (i > 0) count++;
            count += beforeLines[i].length;
        }
        return count + EditorSupport._textLength(node);
    }
    var lines = EditorSupport._lines(blockEl, node);
    var count = 0;
    for (var i = 0; i < lines.length; i++) {
        if (i > 0)
            count++;
 ...
 */
EditorSupport._findBlockEl = function (editorEl, node) {
...
EditorSupport.readSelection = function (editorEl) {
    ...
    var anchorBlockEl = EditorSupport._findBlockEl(editorEl, anchorNode);
    var focusBlockEl = EditorSupport._findBlockEl(editorEl, focusNode);
    ...
    var anchorOff = EditorSupport._offsetInBlock(anchorBlockEl, anchorNode, sel.anchorOffset);
    var headOff = EditorSupport._offsetInBlock(focusBlockEl, focusNode, sel.focusOffset);
 ...
 */
EditorSupport._resolvePosition = function (blockEl, position) {
    if (position <= 0) {
        var first = EditorSupport._firstLeaf(blockEl);
        if (first && first.nodeType === 3) {
            // Don't position inside a CEF element at offset 0.
            var cef = EditorSupport._cefAncestor(blockEl, first);
            if (cef) {
                var idx = EditorSupport._childIndex(cef.parentNode, cef);
    ...
    var n = EditorSupport._firstLeaf(blockEl);
    while (n != null) {
        if (n.nodeType === 3) {
            // Check if this text node is inside a CEF element.
            var cef = EditorSupport._cefAncestor(blockEl, n);
            if (cef) {
                // Treat the CEF element as atomic.
                var cefLen = EditorSupport._textLength(cef);
                var cefIdx = EditorSupport._childIndex(cef.parentNode, cef);
                if (position < cefLen) {
                ...
                n = EditorSupport._nextLeaf(blockEl, EditorSupport._lastLeaf(cef));
                continue;
            }
        ...
        n = EditorSupport._nextLeaf(blockEl, n);
    }
 ...
 */
EditorSupport.setCursor = function (editorEl, blockIndex, offset) {
    var blockEl = editorEl.querySelector('[data-block-index="' + blockIndex + '"]');
    if (!blockEl)
        return;
    var pos = EditorSupport._resolvePosition(blockEl, offset);
    if (!pos)
        return;
    var sel = document.getSelection();
    sel.removeAllRanges();
 ...
 */
EditorSupport.setSelection = function (editorEl, anchorBlock, anchorOffset, headBlock, headOffset) {
    ...
    if (!anchorBlockEl || !headBlockEl)
        return;
    var anchorPos = EditorSupport._resolvePosition(anchorBlockEl, anchorOffset);
    var headPos = EditorSupport._resolvePosition(headBlockEl, headOffset);
 ...
 */
EditorSupport.getInputType = function (evt) {
 ...
 */
EditorSupport.getInputData = function (evt) {
 ...
 */
EditorSupport.getClipboardText = function (evt) {
    if (!evt)
        return null;
 ...
 */
EditorSupport.moveCursorToStart = function (el) {
 ...
 */
EditorSupport.moveCursorToEnd = function (el) {
    var range = document.createRange();
    range.selectNodeContents(el);
 ...
 * is not collapsed.
 */
EditorSupport.cursorOffsetInCell = function (cellEl) {
    var sel = window.getSelection();
    ...
    if (!p)
        return -1;
    return EditorSupport._offsetInBlock(cellEl, n, sel.anchorOffset);
}
 ...
 */
EditorSupport.cellFromSelection = function (editorEl) {
    var sel = window.getSelection();
    if (!sel || sel.rangeCount === 0)
        return null;
    var n = sel.anchorNode;
 ...
 */
EditorSupport.selectionInCell = function (cellEl) {
    var sel = window.getSelection();
    if (!sel || sel.rangeCount === 0)
        return null;
    // Verify anchor is inside cellEl.
    ...
    if (!p)
        return null;
    var anchorOff = EditorSupport._offsetInBlock(cellEl, sel.anchorNode, sel.anchorOffset);
    var focusOff = EditorSupport._offsetInBlock(cellEl, sel.focusNode, sel.focusOffset);
 ...
 * character offsets.
 */
EditorSupport.setSelectionInCell = function (cellEl, from, to) {
    var fromPos = EditorSupport._resolvePosition(cellEl, from);
    var toPos = EditorSupport._resolvePosition(cellEl, to);
    if (!fromPos || !toPos)
        return;
    var sel = window.getSelection();
    sel.removeAllRanges();
