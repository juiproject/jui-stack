...
import com.effacy.jui.ui.client.fragments.Btn.Variant;
...
public class DiagramBlock extends Block {
     ...
     *                the caption.
     */
    public DiagramBlock(IEditor editor, ContentBlock blk) {
        super (BlockType.DIAGRAM, editor, blk);
    }
    ...
    protected void refresh(ContentBlock content) {
        ...
        // Update the caption.
        if (StringSupport.empty (content.caption ())) {
            JQuery.$ (captionEl).hide ();
        } else {
            DomSupport.innerText (captionEl, content.caption ());
    ...
    class EditDialog extends SimpleComponent {
        ...
        protected INodeProvider buildNode(Element el) {
            ...
            ContentBlock content = DiagramBlock.this.content();
            String source = content.contractLines ();
            ...
            return Wrap.$ (el).$ (root -> {
                ...
                Div.$ (root).style (styles.diagram_editor_top ()).$ (outer -> {
                    ...
                    Div.$ (outer).style (styles.diagram_editor_preview ()).$ (preview -> {
                        ...
                        Div.$ (preview).$ (btn -> {
                            Btn.$ (btn, "Preview").icon (FontAwesome.eye ()).variant (Variant.STANDARD_EXPANDED).onclick (() -> {
                                preview ();
                            });
                        });
                    });
