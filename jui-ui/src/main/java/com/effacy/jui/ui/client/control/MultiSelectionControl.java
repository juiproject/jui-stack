...
public class MultiSelectionControl<V> extends Control<List<V>, MultiSelectionControl.Config<V>> {
    ...
    public static class Config<V> extends Control.Config<List<V>, Config<V>> {
         ...
         */
        private BiConsumer<IStore<V>,List<V>> preload;
         ...
         */
        @SuppressWarnings("rawtypes")
        private BiConsumer<ContainerBuilder,List<V>> selectionRenderer;

        /**
         * See {@link #selectionStyle(SelectionStyle)}.
         */
        private SelectionStyle selectionStyle = SelectionStyle.LIST;
         ...
         */
        @Override
        public Config<V> modifiedHandler(IControlModifiedHandler<List<V>> modifiedHandler) {
        ...
        public <S extends IStore<V>> Config<V> store(S store, BiConsumer<S,List<V>> preload) {
            if (store != null) {
                this.store = store;
                if (preload != null)
                    this.preload = (BiConsumer<IStore<V>,List<V>>) preload;
            }
            return this;
        }
         ...
         */
        @SuppressWarnings("rawtypes")
        public Config<V> selectionRenderer(BiConsumer<ContainerBuilder,List<V>> selectionRenderer) {
            this.selectionRenderer = selectionRenderer;
            return this;
        }

        /**
         * Govern how selected items will display (notwithstanding any spefified
         * renderer provided by {@link #selectionRenderer(BiConsumer)}).
         * 
         * @param selectionStyle
         *                       the style to apply.
         * @return this configuration instance.
         */
        public Config<V> selectionStyle(SelectionStyle selectionStyle) {
        ...
        @Override
        @SuppressWarnings("unchecked")
        public MultiSelectionControl<V> build(LayoutData... data) {
            return build (new MultiSelectionControl<V> (this), data);
        }
     ...
     *               the configuration.
     */
    public MultiSelectionControl(MultiSelectionControl.Config<V> config) {
        super (config);
    ...
    /**
     * Allows for the rendering of the selected values to be different from that of
     ...
     *             the root container.
     */
    protected void buildSelectedRenderer(ContainerBuilder<?> root, List<V> value) {
        if (config ().selectionRenderer != null) {
            config ().selectionRenderer.accept (root, value);
        } else if (SelectionStyle.LIST == config().selectionStyle) {
            // Default is to use the value mapper and present as a list.
            Itr.forEach (value, (c,item) -> {
                if (!c.first ())
                    Text.$ (root, ",");
                if (config ().labelMapper != null)
                    Span.$ (root).text (config ().labelMapper.apply (item));
                else
                    Span.$ (root).text ((item == null) ? "[NULL]" : item.toString ());
            });
        } else if (SelectionStyle.CHIP == config ().selectionStyle) {
            Itr.forEach (value, (c,item) -> {
                Span.$ (root).style (styles().chip()).$ (chip -> {
                    buildRenderer (chip, item);
                    Em.$ (chip)
                        .style (FontAwesome.times (), styles ().chip ())
                        .attr("remover", true)
                        .attr("item", c.index ());
                });
            });
    ...
    protected ISelectorMenu<V> createSelector() {
        return new SelectorMenu<V> (new ISelectorMenuConfig<V> () {

            public BiFunction<V,V,Boolean> getComparator() {
            ...
            @Override
            public int getStoreBatchSize() {
                return config ().storeBatchSize;
            }

            @Override
            public boolean isAllowSearch() {
                return config ().allowSearch;
            }

            @Override
            public boolean isStoreClear() {
                return config ().storeClear;
            }
            ...
            @Override
            public Consumer<Consumer<V>> getAddHandler() {
                return config ().addHandler;
            }

            @Override
            public String getAddLabel() {
                return config ().addLabel;
            }

            @Override
            public boolean isOverflowSafe() {
                return config ().overflowSafe;
            }
            ...
            protected void buildItem(ContainerBuilder el, V value) {
                if (MultiSelectionControl.this.config().optionRenderer != null)
                    MultiSelectionControl.this.config().optionRenderer.accept(el, value);
                else
                    MultiSelectionControl.this.buildRenderer (el, value);
            }

            @Override
            protected boolean filterMatch(String keywords, V value) {
                return MultiSelectionControl.this.filterMatch (keywords, value);
            }

            @Override
            protected void onSelect(V value) {
                MultiSelectionControl.this.select (value);
            }

            @Override
            protected void onDeselect(V value) {
                MultiSelectionControl.this.deselect (value);
            }
     ...
     *              the value that was selected.
     */
    public void deselect(V v) {
        int idx = (v == null) ? -1 : _indexOf (v);
        if (idx >= 0) {
            value.remove (idx);
            valueToSource (value);
        }
    ...
    /**
     * Determines the index of an item (using the comparator from the configuration
     * if supplied).
     * 
     * @param item
     *             the item to check.
     ...
     */
    protected int _indexOf(V item) {
        if (config ().comparator == null)
            return value.indexOf (item);
        int idx = 0;
        for (V sel : value) {
            if (config ().comparator.apply (sel, item))
                return idx;
            idx++;
        }
        return -1;
    ...
    public void valueToSource(List<V> value) {
        // If the passed value is the same as our internal value list, then we are
        // modifying internally and this is really just an update.
        if (value != this.value) {
            this.value.clear ();
            if (value != null)
        ...
        if (this.value.isEmpty ()) {
            ...
            Wrap.buildInto(contentEl, root -> {
                Span.$ (root)
                    .text (StringSupport.empty (config ().placeholder) ? "- Select -" : config ().placeholder);
            });
        } else {
            getRoot ().classList.remove (styles ().empty ());
            Wrap.buildInto (contentEl, root -> buildSelectedRenderer (root, this.value));
        }
    }
    ...
    protected INodeProvider buildNode(Config<V> data) {
        return DomBuilder.div (inner -> {
            ...
            if (SelectionStyle.CHIP == config ().selectionStyle)
                inner.style (styles ().chip ());
            if (Debug.isTestMode())
            ...
            inner.on (e -> {
                ...
                if (e.isEvent(UIEventType.ONCLICK) && (e.getTarget().getAttribute ("remover") != null)) {
                    // The "remover" is on a chip to remove that specific item.
                    int idx = Integer.parseInt (e.getTarget().getAttribute ("item"));
                    deselect (value.get (idx));
                } else {
            ...
            Div.$ (inner).$ (selector -> {
                ...
                selector.apply (attach (MultiSelectionControl.this.selector));
                if (data.selectorTop)
            ...
            Div.$ (inner).id ("content").by ("content");
            Em.$ (inner).$ (icon -> {
                icon.on (e -> {
                    e.stopEvent ();
                    hideSelector ();
    ...
    @CssResource({
        ...
        "com/effacy/jui/ui/client/control/MultiSelectionControl.css",
        "com/effacy/jui/ui/client/control/MultiSelectionControl_Override.css"
    })
