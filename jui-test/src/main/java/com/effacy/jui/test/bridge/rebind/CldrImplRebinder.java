...
public class CldrImplRebinder implements Rebinder {

    public static String LOCALE = "en";

    private Class<? extends CldrImpl> dKlass;

    @Override
    @SuppressWarnings("unchecked")
    public <T> Optional<T> create(Class<T> klass) throws Exception {
        if (!CldrImpl.class.equals(klass))
            return Optional.empty();
        if (dKlass == null) {
            dKlass =  new ByteBuddy()
                .subclass(CldrImpl.class)
                .make()
                .load(CldrImpl.class.getClassLoader())
                .getLoaded();
        }
