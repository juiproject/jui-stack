...
import com.effacy.jui.codeserver.gwt.JobEvent.Status;
...
public class Job {
    ...
    private static final ConcurrentMap<String, AtomicInteger> prefixToNextId = new ConcurrentHashMap<String, AtomicInteger>();
    ...
    // Listeners
    private final CompilerModule outbox;
     ...
     */
    Job(CompilerModule box, Map<String, String> bindingProperties, TreeLogger parentLogger, Options options) {
        this.id = UUID.randomUUID().toString();//chooseNextId(box);
        this.outbox = box;
        this.inputModuleName = box.inputModuleName();
     ...
     */
    public CompilerModule getOutbox() {
        return outbox;
    }
     ...
     */
    public synchronized void onSubmitted(JobEventTable table) {
        if (wasSubmitted()) {
     ...
     */
    public synchronized void onStarted(int compileId, CompileDir compileDir) {
