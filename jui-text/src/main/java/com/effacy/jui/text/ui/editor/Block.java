...
import com.effacy.jui.text.ui.editor.Editor.IEditorCSS;
...
public abstract class Block implements IUIEventHandler {
     ...
     */
    private FormattedBlock content;
     ...
     *                seeding content.
     */
    protected Block(BlockType type, IEditor editor, FormattedBlock content) {
        this.type = type;
        this.editor = editor;
     ...
     * @return the content.
     */
    public final FormattedBlock content() {
        if (this.content == null) {
            this.content = new FormattedBlock (type);
     ...
     */
    public void update(FormattedBlock content) {
        if (content == null)
            return;
        if (this.type != content.getType ()) {
            Logger.error ("Block type is " + type.name() + " but passed content was " + content.getType());
            return;
        }
        this.content = content;
        refresh ();
     ...
     */
    protected abstract void refresh(FormattedBlock content);
    ...
    protected void refreshIndent() {
        if (indent < 0)
            indent = (content != null) ? content.getIndent() : 0;
